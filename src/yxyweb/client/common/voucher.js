...
const voucher = function () {
  ...
  const _abandon = function (viewmodel, afterAct) {
    if (viewmodel.getParams().tplid || viewmodel.getCache('entryMode') !== env.VOUCHER_STATE_BROWSE) {
      viewmodel.communication({ type: 'return' });
      return;
    }
  ...
  const localPrint = function (billNo, viewModel, params, beforeAct, afterAct) {
    ...
    if (templatesModel) {
      ...
      _localPrint(billNo, templateCode, id, route, beforeAct);
    } else {
      ...
      proxy.getTemplate({ billno: billNo }, function (err, data) {
        ...
        _localPrint(billNo, template.templatecode, id, route, beforeAct);
      });
    }
  }

  const _localPrint = function (billno, templateCode, id, route, beforeAct) {
    const proxy = cb.rest.DynamicProxy.create({
    ...
    beforeAct(beforeActData, function () {
      let callback = function (err, res) {
        ...
        if (!err) {
          ...
          if (currentAction !== 'saveandaudit' && viewmodel.getParams().saveReturn !== false && (viewmodel.getParams().tplid || viewmodel.getCache('entryMode') !== env.VOUCHER_STATE_BROWSE)) {
