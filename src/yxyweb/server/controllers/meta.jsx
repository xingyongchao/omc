...
export const getMeta = async (ctx) => {
  ...
  if (meta.code == 200) {
    ...
    if (tplHtml.tpl) {
      results = _combineTplMeta(tplHtml.tpl, meta);
    } else {
      results = {
        code: 404,
        message: tplHtml.msg
 ...
 * meta核心
 */
const _combineTplMeta = (tpl, meta) => {
  ...
  viewmeta.extendFileName = _getExtendVmFileName(viewmeta);
  viewmeta.extendVmName = _getExtendVmName(viewmeta);
...
const _getFreeViewMeta = (vmmeta) => {
  ...
  let tableEntites = vmmeta.entities && vmmeta.entities.filter(x => {
    return x.cModelType && x.cModelType.toLowerCase() == 'table' && x.cCode
  });
  let treeEntites = vmmeta.entities && vmmeta.entities.filter(x => {
    return x.cModelType && x.cModelType.toLowerCase() == 'tree'
  });
  let parentCodes = new Array();
  //除了tree 和table 的entity
  vmmeta.entities.forEach(function (val) {
    if (!val.cModelType || (val.cModelType && val.cModelType.toLowerCase() != 'tree' && val.cModelType && val.cModelType.toLowerCase() != 'table')) {
  ...
  treeEntites && treeEntites.forEach(function (treeEntity) {
  ...
  subEntities.forEach(function (entity) {
    ...
    if (modelType === 'TagModel') {
      ...
      if (fields.length || entity.children)
        obj.columns = _recursive({ fields, children: entity.children });
      if (checkField)
 ...
 * 获取extend js的文件名
 */
const _getExtendVmFileName = (viewmeta) => {
  let vmName = viewmeta.vmName;
  let extendFileName = vmName + '.Extend.js';
 ...
 * 获取extend 变量名
 */
const _getExtendVmName = (viewmeta) => {
  let vmName = viewmeta.vmName;
  let extendName = vmName + '_Extend';
  return extendName;
}
