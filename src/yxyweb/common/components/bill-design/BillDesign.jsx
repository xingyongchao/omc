  ...
  getModalControl = (modalData) => {
    ...
    if (readOnly) {
      buttonControl = (
        ...
        // <Button key="edit" model={this.editModel} value="编辑" icon="edit" onClick={() => this.onButtonClick('edit')} />
      )
    } else {
      ...
      if (hasDataSource) {
        ...
        buttonControl = (
          <div>
            ...
            {/* <Button key="moveup" model={this.upModel} value="上移" icon="shangyi" onClick={() => this.onMoveUp()} />
            <Button key="movedown" model={this.downModel} value="下移" icon="xiayi" onClick={() => this.onMoveDown()} /> */}
          </div>
        )
      } else {
        buttonControl = (
    ...
    if (modalData && modalData.length > 1) {
      modalData.map(ele => {
        // let className = '';
        // if (activeKey == ele.groupId) className = "bill-design-tab-active"
        // paneControls.push(
        //   <div className={className} key={ele.groupId} onClick={() => this.onTabsChange(ele.groupId)}>{ele.cName}</div>
        // );
