...
class EChartDesign extends React.Component {
  constructor(props) {
    ...
    let config = props.groupConditionState[this.state.billnum].eChart.eChartConfig;
    this.bSetTreeModel = false;
    if (config && config[props.chartKey] && (_.isEmpty(config[props.chartKey]) == false)) {
      this.state.chart_Backup = config[props.chartKey];
      eChartCommon.upgradeConfig(config[props.chartKey]);
      if (_.get(config[props.chartKey], "yySetting.type") == "scatter") {
        this.SetTreeModel(config[this.state.chartKey]);
      }
    }
    else {
      this.state.chart_Backup = undefined;
      this.chooseChart("bar", "1");
    }
  ...
  getChartSettingPanel() {
    ...
    let config = groupConditionState[this.state.billnum].eChart.eChartConfig[this.state.chartKey];
        ...
        <Row className="eChartSetting_Content" colCount={12}>
          ...
          <Col span={10} className="eChartSetting_ContentCol2" >
            <div className="eChartSet-mainwnumber">
              <Input
                disabled={false}
                placeholder="请输入1-20数字"
                value={_.get(config, "yySetting.orderInfo.topNum")}
                onChange={(e) => self.setEChartConfigValue("yySetting.orderInfo.topNum", e.target.value, true, true)} className="eChartSetinput"
              />
            </div>
          </Col>
        </Row>);
  ...
  setEChartConfigValue(fieldPath, fieldValue, bCheckNumber, bInterger) {
    const { groupConditionState, groupConditionRedux } = this.props;
    if (bCheckNumber && fieldValue.trim() != "") {
      if (isNaN(fieldValue) == true || Number(fieldValue) < 0 || Number(fieldValue) > 100) {
  ...
  chooseChart(eChartType, eChartSubType) {
    const { groupConditionState, groupConditionRedux } = this.props;
    ...
    groupConditionRedux.eChartSetting_ChooseChart(this.getBillNum(), configTemplate);
    this.setFocus();
  }
  getBillNum() {
  ...
  getChart() {
    const { groupConditionState, groupConditionRedux } = this.props;
    return groupConditionState[this.state.billnum].eChart.eChartConfig[this.state.chartKey];
  }
  ...
  eChartSettingOK = () => {
    ...
    const { groupConditionState, groupConditionRedux } = this.props;
    let config = groupConditionState[this.state.billnum].eChart.eChartConfig[this.state.chartKey];
    eChartCommon.LogChartInfo("设置界面 点击确定 getChartSettingPanel config", JSON.stringify(config), 900);
    groupConditionRedux.eChartSetting_EChartSettingOK(this.getBillNum(), this.state.chartKey);
  }

  eChartSettingCancel = () => {
    const { groupConditionState, groupConditionRedux } = this.props;
    groupConditionRedux.eChartSetting_CancelChartConfig(this.getBillNum(), this.state.chart_Backup);
  }
  shouldComponentUpdate(nextProps, nextState) {
    // if (this.props.bCancelSetting == false && nextProps.bCancelSetting == true) {
    //   this.eChartSettingCancel();
    //   return false;
    // }
    // else {
    //   return true;
