...
import { Input, Icon, Button, Checkbox, Radio, Carousel, Tabs } from 'antd';
...
export default class eChartCarousel extends React.Component {
  ...
  render() {
    ...
    if (carouselTemplates.length == 0) {
      ...
      eChartCommon.LogChartInfo("大屏轮播: eChartCarousel Render isPreview = " + self.state.isPreview + " clientWidth & clientHeight", JSON.stringify(style_outer), 15);
  ...
  componentDidMount() {
    ...
    // 列表中预览传入格式：http://localhost:3003/echartcarousel?isPreview=1&previewId=5875&type=1
    // this.state.isPreview = (_.get(self.props, "location.query.isPreview") == 1
    //   || _.get(self.props, "location.search").toString().indexOf("isPreview=1") >= 0) ? true : false;
    this.state.isPreview = false;
    let previewId = 0;
    let type = -1;
    var params = new cb.utils.queryString(self.props.location.search);
    if (params && params.query) {
      this.state.isPreview = params.query && params.query.isPreview == "1";
      previewId = params.query && params.query.previewId;
          ...
          let callback = (json) => {
            if (json.code === 200) {
              if (json.data && json.data.length > 0) {
                ...
                if (self.state.isPreview == false && carouselTemplates.length > 1)
                  self.setAutoPlay();
              }
              else {
