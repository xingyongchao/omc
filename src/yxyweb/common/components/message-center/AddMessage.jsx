...
class SubscribeUser extends Component {
    render() {
        ...
        this.props.receivers.forEach((item)=>{
            receivers.push(<Option value={item.value}>{item.text}</Option>)
        })
        return <Select optionFilterProp="children" className={this.props.className}
            mode="multiple" value={this.props.userId} onChange={this.props.onChange}
        >{receivers}
        </Select>
    }
...
class AddMessage extends Component {
    ...
    getFormControl = () => {
        ...
        const { name, stopstatus,startTime,endTime,receivers, userId,frequency, interval, timepoint, day, operatorData, conditionDesc, context, errorInfo } = this.props.addMessage;
