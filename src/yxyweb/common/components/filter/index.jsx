...
export default class ConvenientQuery extends React.Component {
  constructor(props) {
    super(props);
    const filterId = props.config && props.config.filterid;
    this.state = {
      schemeData: [],
      current: '',
      schemeName: '',
    ...
    this.state.isInPanelDesign = params.isInPanelDesign ? params.isInPanelDesign : false;
    // debugger;
    this.vm = cb.loader.initMetaCommonViewModel(
      'FilterViewModel',
      'filterViewModel',
      {
        filterId: filterId || params.filterId,
        condition: params.condition,
        cardKey: params.cardKey,
        isInPanel: this.state.isInPanel,
        isInPanelDesign: this.state.isInPanelDesign,
  ...
  render() {
    ...
    if (this.state.tag2InFirstRow == false || (commonFilterCount > 0 && showMore) || this.props.autoExpand) {
      if (this.state.isInPanelDesign == false) {
    ...
    if (this.state.filterModel.length > rowNum && !this.props.autoExpand && this.state.isInPanel == false && this.state.isInPanelDesign == false) {
    ...
    return (
      <Row className={`${this.props.config && this.props.config.classname} ${this.props.autoExpand === false && 'voucher-filter'}`}>
        <Col className='filter-controls'>{control}</Col>
    ...
    this.state.filterModel.forEach(function (ele, index) {
      ...
      if (ele.isCommon == 1 || ele.isCommon == true) {
        ...
        if (ctrlType === 'tagbutton') {
          ...
          control = (
            <Row key={ele.itemName}>
              <ComName model={model} isInPanel={this.state.isInPanel} cShowCaption={ele.cShowCaption} isInFilterJSX={true} />
            </Row>
          );
        }
        else {
