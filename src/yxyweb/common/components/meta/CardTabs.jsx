...
export default class CardTabs extends Component {
  ...
  renderProgressBar(containers, activeMode) {
    ...
    this.progressbar = <ul className="inventory-process">{items}</ul>
  }
  ...
  renderTabItems(containers) {
    ...
    containers.forEach((container, index) => {
      ...
      items.push(<TabPane tab={container.cName} key={key} disabled={disabled}><div id={`${viewModel.getParams().billNo}|${key}`}>{tabContent}</div></TabPane>);
    });
    return items;
  }
  render() {
