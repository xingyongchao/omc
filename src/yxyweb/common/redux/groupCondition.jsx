      ...
      if (json.code === 200) {
        ...
        if (id) {
          ...
          if (metaData.displayStyle) {
            ...
            chart.eChartConfig = JSON.parse(metaData.chartConfig);
          }
...
export function editCondition_SetIsCrossTable(billnum, fieldName, fieldValue) {
  return function (dispatch, getState) {
    ...
    dispatch(genAction('PLATFORM_UI_GroupCondition_EditCondition_SetValue', param));
    dispatch(genAction('PLATFORM_UI_EChartSetting_SetChartConfig', { billnum, bClose: true }));
  }
}
...
export function eChartSetting_EChartSettingOK(billnum, chartKey) {
  return function (dispatch, getState) {
    ...
    if (_.isEmpty(errors) == true) {
      dispatch(genAction('PLATFORM_UI_EChartSetting_Show', { billnum, bShow: false }));
    }
    else {
  ...
  if (eChart.displayType == 2 || eChart.displayType == 3) {
    ...
    if (yySetting.type == "ranktable") {
      if (
        ...
        || Number(yySetting.orderInfo.topNum) > 20
        || Number(yySetting.orderInfo.topNum) < 1
      ) {
        errors.topNum = "请录入1-20的整数。";
      }
    }
...
export function eChartSetting_Show(billnum, bShow) {
  return function (dispatch, getState) {
    dispatch(genAction('PLATFORM_UI_EChartSetting_Show', { billnum, bShow }));
  }
}
...
export function eChartSetting_ChooseChart(billnum, configTemplate) {
  return function (dispatch, getState) {
    // dispatch(genAction('PLATFORM_UI_EChartSetting_SetChartConfig', { billnum, config: configTemplate, bClose: false }));
    ...
    dispatch(genAction('PLATFORM_UI_EChartSetting_SetChartConfig', { billnum, config: newEChart, bClose: false }));
  }
}
export function eChartSetting_CancelChartConfig(billnum, chart_Backup) {
  return function (dispatch, getState) {
    dispatch(genAction('PLATFORM_UI_EChartSetting_SetChartConfig', { billnum, config: chart_Backup, bClose: true }));
  }
}
