...
export function editRow(index){
  return async function (dispatch, getState) {
        ...
        // dispatch(initData(billno, menucode, searchcondition, groupschemaid, data, entityname, gridmodel));
...
export function onOk() {
  return async function (dispatch, getState) {
    const { editId,name, receivers,gridModel, subscriptionCondition,user, conditionDesc, context, stopstatus, day, timepoint, frequency, interval, startTime, endTime } = getState().addMessage.toJS();
    ...
    dispatch(getListData(gridModel));
    dispatch(genAction('PLATFORM_UI_ADD_INIT_DATA', { visible:false }))};
}
...
export function getReceivers() {
  return function (dispatch, getState) {
    ...
    proxy(config).then(json => {
      ...
      dataSource.forEach(
        (item, index, arr) => {
          receivers.push({ text: item.name, value: item.id })
        }
      )
...
export function initData(billno, menucode, searchcondition, groupschemaid, data, entityname, gridmodel) {
  return async function (dispatch) {
    if (billno !== billNo) {
      billNo = billno;
      gridModel = gridmodel;
